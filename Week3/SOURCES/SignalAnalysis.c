...
/*****************************************************************************/
{
  ...
  FILE *fp2 = fopen("Question 2a.txt", "a");
  FILE *fp3 = fopen("Question 2b.txt", "a");
  ...
  for(i=0; i<=DFT_LENGTH/2; i++){
    ...
    logspectralval = log(Abs_Fourier[i]);
    fprintf(fp2, "%f\n", logspectralval);
...
/*****************************************************************************/
{
  ...
  /* open feature file */
  if((fp_feat=fopen(FeatureFile, "w"))==NULL) {
    fprintf(stderr, "\nError: cannot open %s\n",FeatureFile);
    exit(EXIT_FAILURE);
  }
  ...
  {
    fprintf(fp1,"%d\t%f\n", Samples[N_smp],1.0*N_smp/SAMPLE_RATE);
    N_smp++;
  } 
